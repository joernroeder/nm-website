...
"use strict";(function($) {
  ...
  JJMarkdownEditor = (function() {
    ...
    JJMarkdownEditor.prototype.dragAndDropSetup = function() {
      ...
      return _bindDropHandler = function() {
        ...
        return _this.currentDrag.$dropzone.on('drop', function(e) {
          var $dropzone, $el, $progressBar, $target, errorMsg, files, formData, hideDropzoneTimeout, req, _xhrProgress;
          ...
          if (_this.inlineElementDragged) {
            ...
            if ($target.is($preview)) {
              _this.moveElementToEditorBottom($el);
            } else {
              _this.moveElementAbove($el, $target);
            }
            $dropzone.remove();
            _this.parseMarkdown();
          } else {
    ...
    JJMarkdownEditor.prototype.moveElementAbove = function($el, $target) {
      var mdTag, pos;

      mdTag = $el.data('md-tag').replace(/\\/g, '');
      pos = $el.data('editor-pos') + mdTag.length;
      this.insertAtEditorPos($target, mdTag);
      return this.removeAtEditorPos(pos, mdTag);
    };

    JJMarkdownEditor.prototype.moveElementToEditorBottom = function($el) {
      var mdTag, pos;

      mdTag = $el.data('md-tag').replace(/\\/g, '');
      pos = $el.data('editor-pos');
      this.removeAtEditorPos(pos, mdTag);
      return this.$input._val(this.$input._val() + mdTag);
    };
